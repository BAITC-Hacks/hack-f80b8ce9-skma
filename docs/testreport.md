# Повторная визуальная проверка с реальным OpenAI API — 23.09.2026

## Актуальный прогон версии 71075fc

По прямому запросу пользователя backend перезапущен с переданным им OpenAI API-ключом в окружении процесса. Использованы `https://api.openai.com/v1` и модель `gpt-5.4-mini`. Значение ключа не записывалось в отчёт или репозиторий. Frontend: http://127.0.0.1:3000/, backend: http://127.0.0.1:8000/, база — SQLite в памяти. Отдельных учётных данных EKT нет.

Это новый прогон с LLM. Предыдущий раздел ниже описывает прогон в режиме правил и оставлен как история. Во время самого прогона использовался неизменённый код 71075fc. При публикации отчёта обычный push был отклонён, поскольку в origin/main появились новые коммиты. После завершения тестов выполнен git pull --rebase до 4e1ebc3; конфликтов не было. Внешние изменения включают backend и поддержку казахского языка. Они не входят в описанный визуальный прогон; работающий backend сохраняет загруженную версию 71075fc до перезапуска. Проверки проходили примерно в 17:32–17:37 UTC+05:00.

### Реальные действия через браузер

| Сценарий | Наблюдаемый результат | Статус |
|---|---|---|
| Открыть главную и чат на настольном экране | Главная, инструкция, панель и поле ввода отображаются; чат открывается | Пройден |
| «Какие условия доставки и оплаты? Ответь кратко, тремя пунктами» | Получены именно три пункта; POST /api/chat → 200 | Ответ LLM получен, есть замечания ниже |
| Мобильный viewport 390×844 | Чат занимает экран; поле, скрепка, отправка и закрытие видны; длинный артикул переносится | Пройден в эмуляции размера |
| Уточнить доставку при ровно 15 000 ₸ в Алматы | Модель отличила «свыше» от «ровно» и ответила, что бесплатная доставка не применяется | Пройден для условия бесплатности |
| Загрузить examples/spec-example.csv из README через скрепку | POST /api/chat/upload → 503; показаны «Каталог временно недоступен.» и «Повторить» | Доступ к каталогу заблокирован; UI корректно показывает ошибку |
| После ошибки загрузить unknown-product.csv | HTTP 200; «1 позиция», «Не найдено»; предыдущий alert исчез | Пройден |
| Спросить артикул и количество из последнего файла | Модель вернула ZZZ-NO-SUCH-PRODUCT-RETEST-999999 и 2 шт.; инструкция отвечать по-русски выполнена | Пройден |
| Уточнить ровно 30 000 ₸ для Астаны и 400 000 ₸ для города без филиала | Модель уточнила: бесплатно только свыше этих сумм | Пройден после уточнения; исходная формулировка неточна |

Все четыре текстовых обращения завершились HTTP 200. В журнале текущего backend за эти обращения не было предупреждений `LLM failed, using rule-based answer`. Ответы выполняют произвольную просьбу о трёх пунктах и используют контекст загруженного файла; это соответствует активному LLM-пути, а не прежнему шаблонному ответу. Отдельные идентификаторы запросов OpenAI, токены и стоимость не собирались. Загрузка и разбор CSV сами по себе не являются вызовом LLM; LLM проверялась последующим вопросом о содержимом.

### Обнаруженные проблемы

1. **Оформление ответа:** `**...**` выводится в пузырях буквально, без жирного начертания. Текст читается, но Markdown ухудшает внешний вид. Воспроизводится на desktop и mobile; исправление в этом прогоне не вносилось.
2. **Точность порогов:** первый ответ содержит «от 30 000 ₸» и «от 400 000 ₸», тогда как локальный `backend/data/terms.md` говорит «свыше». Последующее уточнение исправило ответ, но исходная неточность остаётся дефектом качества. Сравнение выполнено с локальным справочником, актуальность коммерческих условий на сайте партнёра не проверялась.
3. **Каталог EKT недоступен:** реальный OpenAI-ключ не предоставляет доступ к API партнёра. CSV с реальными товарами возвращает 503. Карточки, цены, остатки, аналоги и полный пользовательский путь наполнения корзины не подтверждены.

В четырёх ответах этого прогона случайного смешения русского с другими языками не наблюдалось; это ограниченная выборка, а не доказательство устранения прежнего замечания. При ровно 15 000 ₸ модель назвала доставку платной; локальный справочник явно определяет условие бесплатности, но отдельно не уточняет тариф для точного равенства, поэтому сумма платы не подтверждалась.

### Автоматические проверки

- Backend pytest: **71 passed in 2.20s**.
- Ruff check: **All checks passed**; format: **44 files already formatted**.
- ESLint: **0 ошибок, 2 прежних предупреждения** об анонимном default export в eslint.config.mjs и postcss.config.mjs.
- TypeScript tsc --noEmit: **успешно**.
- `make` отсутствует; составляющие `make check` выполнены напрямую. `npm.cmd` также не найден в PATH, поэтому ESLint и TypeScript запущены через bundled node.exe и локальные установленные пакеты.
- Production build заново не запускался: исходный код не менялся; успешная сборка предыдущего прогона описана ниже.
